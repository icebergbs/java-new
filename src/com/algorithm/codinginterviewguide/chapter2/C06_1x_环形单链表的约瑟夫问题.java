...
入，老编号=（新编号+(m-1)%i)%i+1
  我们可以假设m-1=k*i+b(k>=0),那么，
        （新编号+（m-1)%i)%i+1=(新编号+b)%i+1
  再看这个式子，（新编号+m-1)%i+1,如果把m-1=k*i+b带入，
        （新编号+m-1)%i+1=(新编号+b)%i+1
  所以原来的等式可以进一步简化为，
         老编号=（新编号+m-1)%i+1
  至此，已经求出了杀死一共节点前后编号的变化函数，所以整个解法的过程总结为：
  1. 遍历链表，求链表的节点个数记为n, 时间复杂度为O(N)
  2. 根据n和m的值，还有上文分析的Num(i-1)(新编号）和Num(i)（老编号）的关系，
依次求生存节点的编号。这一步的具体过程请参看代码中的getLive(), getLive()为单
决策的递归函数，且递归为N层，所以时间复杂度为O(N)
 ...
 */
    ...
    /**
     * 进阶问题：
     *    8个节点，被杀死的是编号4，新老编号对应关系如下：
     *   老编号：1 2 3 4 5 6 7 8 9
     *   新编号：6 7 8 无 1 2 3 4 5
     *   新编号：2 3 4 无 5 6 7 无 1
     *   新编号：5 6 无 无 1 2 3 无 4
     *   新编号：1 2 无 无 3 4 5 无 无
     *   新编号：2 3 无 无 4 无 1 无 无
     *   新编号：2 3 无 无 无 无 1(4) 无 无
     *   新编号：1(3) 2(4) 无 无 无 无 无 无 无
     *   新编号：1 无 无 无 无 无 无 无 无
     * @param head
     * @param m
     * @return
     */
